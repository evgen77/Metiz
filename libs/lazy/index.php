...
<head>
    ...
    $style = '<style>#sp-left .sp-module:first-child {display: none!important;} #sp-left .sp-module.catalog_list {margin-top: 0;}</style>';
    $url = explode('/', $_SERVER['REQUEST_URI']);
    if ($url[1] == 'ru') {
        if ($url[2] == 'catalog' && $url[3] == NULL) {
            echo $style;
        }
    }
    else {
        if ($url[1] == 'catalog' && $url[2] == NULL) {
            echo $style;
        }
    }
    ?>
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-131897432-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());
  gtag('config', 'UA-131897432-1');
</script>
</head>
